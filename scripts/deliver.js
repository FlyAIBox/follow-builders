...
//
// 中文维护说明：
// 这个脚本只处理“已经写好的摘要”如何送达，不负责抓取、筛选或总结内容。
// 输入可以来自 stdin、--message 或 --file；输出目的地由用户本地配置决定。
// OpenClaw/Codex 场景通常用 stdout，把最终消息交给上层 agent 渠道发送。
// ============================================================================
...
async function getDigestText() {
  ...
  // Check --file flag
  // 适合先把 LLM 输出落盘，再由 cron 或 CI 统一发送。
  ...
  // Read from stdin
  // 默认路径：prepare -> LLM 总结 -> pipe 到 deliver。
  const chunks = [];
...
async function sendTelegram(text, botToken, chatId) {
  ...
  // 这里保留 4000 的安全余量，避免 Markdown 或多字节内容导致边界错误。
  const MAX_LEN = 4000;
  const chunks = [];
  let remaining = text;
  while (remaining.length > 0) {
    ...
    // 优先按段落切分，减少长摘要在 Telegram 中被截成难读的半句话。
  ...
  for (const chunk of chunks) {
    ...
    if (!res.ok) {
      ...
      // LLM 生成的 Markdown 偶尔会有 Telegram 不接受的转义字符。
      // 失败时降级为纯文本发送，比直接丢弃整期摘要更可靠。
...
async function sendEmail(text, apiKey, toEmail) {
  // Resend 的免费域名适合个人摘要；生产化部署时可以改成自有域名发件人。
...
async function main() {
  // Load env and config
  // delivery token 存在 ~/.follow-builders/.env，避免把私钥写进 skill 仓库。
  ...
  if (!digestText || digestText.trim().length === 0) {
    // 空摘要通常意味着上游没有新内容或 LLM 没有产出；这里显式返回 skipped，
    // 方便自动化任务判断不是交付系统故障。
